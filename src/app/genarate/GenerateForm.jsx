...
export default function GenerateForm() {
  ...
  const nextStep = async () => {
    if (currentStep === 1 && !formData.occasion) return alert("Please select an occasion.");
    if (currentStep === 2 && formData.forWho === "specific" && !formData.recipientName) return alert("Please enter a name.");
    if (currentStep === 3 && formData.creationMethod === "write_for_us" && !formData.storyText) return alert("Please provide a story.");
    if (currentStep === 3 && formData.creationMethod === "own_lyrics" && !formData.lyrics) return alert("Please paste your lyrics.");
    if (currentStep === 5 && !formData.email) return alert("Please enter an email address.");
    if (currentStep === 6 && (!formData.genre || !formData.voice || !formData.mood)) return alert("Please complete all music style selections.");
    
    // Check if we are at the final step to submit to database
    if (currentStep === totalSteps) {
      if (!formData.agreeTerms) return alert("Please agree to the Terms and Conditions.");
      ...
      try {
        ...
        alert("Order saved successfully to Database! Redirecting to checkout...");
